...
if obj.continuous_color_options.active
    ...
    if isempty(draw_data.z)
        ...
        if iscell(x)
            for k=1:length(x)
                if iscell(draw_data.continuous_color) %Within-line continuous color given
                    hndl=patch(obj.current_facet_axes,... % Added 12-11-2021 PMA
                        [shiftdim(x{k}) ; flipud(shiftdim(x{k}))],...
        ...
        if iscell(draw_data.x)
            for k=1:length(draw_data.x)
                if iscell(draw_data.continuous_color) %Within-line continuous color given
                    hndl=patch(obj.current_facet_axes,... % Added 12-11-2021 PMA
            [shiftdim(draw_data.x{k}) ; flipud(shiftdim(draw_data.x{k}))],...
            ...
            hndl=patch(obj.current_facet_axes,... % Added 12-11-2021 PMA
            [shiftdim(draw_data.x) ; flipud(shiftdim(draw_data.x))],...
    ...
    if isempty(draw_data.z)
        ...
        hndl=line(obj.current_facet_axes,... % Added 12-11-2021 PMA
            combnan(x),combnan(y),'LineStyle',draw_data.line_style,'lineWidth',draw_data.line_size,'Color',draw_data.color);
    else
        hndl=line(obj.current_facet_axes,... % Added 12-11-2021 PMA
            combnan(draw_data.x),combnan(draw_data.y),combnan(draw_data.z),'LineStyle',draw_data.line_style,'lineWidth',draw_data.line_size,'Color',draw_data.color);
    end
end
